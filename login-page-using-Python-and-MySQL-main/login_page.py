...
class login_page:
    def __init__(self, root):
        self.window = root
        self.window.title("Log In vedraj")
        # Set the window size
    ...
    def login_func(self):
        if self.email_entry.get()=="" or self.password_entry.get()=="":
            ...
            try:
                connection=psycopg2.connect(host=cr.host, user=cr.user, password=cr.password, database=cr.database)
                ...
                if row == None:
                    ...
                    messagebox.showinfo("Success","Wellcome",parent=self.window)
